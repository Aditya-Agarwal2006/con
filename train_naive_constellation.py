...
class ConstellationFeatureExtractor(BaseFeaturesExtractor):
    ...
    def forward(self, observations):
        ...
        sat_obs = observations[:, 1:361].reshape(b, self.n_sats, self.sat_feat) # (b, 24, 15)
        # Apply shared MLP across sats
        sat_rep = self.sat_net(sat_obs.reshape(b * self.n_sats, self.sat_feat))
        sat_rep = sat_rep.reshape(b, self.n_sats, 64)
...
def main():
    ...
    env = DummyVecEnv([lambda: ConstellationEnv(naive_thermal=True)])
    ...
    checkpoint_callback = CheckpointCallback(
        save_freq=100000,
        save_path='models_naive/',
        name_prefix='ppo_naive'
    )
    
    # Logging callback
    log_callback = ConstellationLogCallback()
    
    print("Starting naive training for 10k steps (baseline)...")
    model.learn(
    ...
    model.save("models_naive/final_model")
    env.save("models_naive/vec_normalize.pkl")
